...
jest.mock('@/components/Modals/Modal_2_ClassTaskModal', () => {
  ...
  return function MockClassTaskModal(props: any) {
    ...
    const [showClassworkCard, setShowClassworkCard] = React.useState(true);
    if (!props.visible) return null;
    return (
      <View>
        ...
        {showClassworkCard ? (
          <>
            <Text>Classwork Card</Text>
            <TouchableOpacity onPress={() => setShowOptions(!showOptions)}>
              <Text>More</Text>
            </TouchableOpacity>
            {showOptions ? (
              <TouchableOpacity onPress={() => setShowClassworkCard(false)}>
                <Text>Delete</Text>
              </TouchableOpacity>
            ) : null}
          </>
  ...
  return function MockTaskModal(props: any) {
    ...
    return (
      <>
        ...
        <TouchableOpacity onPress={() => props.clickedNext('Classwork')}>
          <Text>Create</Text>
        </TouchableOpacity>
      </>
    );
...
jest.mock('@/components/Modals/ClassworkModal', () => {
  ...
  return function MockClassworkModal({ visible }: any) {
...
describe('Classwork creation flow', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    const mockState = {
      classes: {
